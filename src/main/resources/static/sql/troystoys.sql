...
CREATE TABLE `product` (
	...
	KEY `fk_brand` FOREIGN KEY (`productBrand`) REFERENCES `brand` (`brandId`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8;
...
CREATE TABLE `transaction` (
	...
	KEY `fk_trans_customer` (`customer_id`),
	CONSTRAINT `fk_trans_customer` FOREIGN KEY (`customer_id`) REFERENCES `customer` (`userId`) ON DELETE CASCADE ON UPDATE CASCADE,
	KEY `fk_trans_product` (`product_id`),
	CONSTRAINT `fk_trans_product` FOREIGN KEY (`product_id`) REFERENCES `product` (`productId`) ON DELETE CASCADE ON UPDATE CASCADE,
